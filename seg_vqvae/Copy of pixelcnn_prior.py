...
from modules import VectorQuantizedVAE, SegmentationModel, GatedPixelCNN
...
def train(data_loader, model, seg_model, prior, optimizer, args, writer):
...
def test(data_loader, model, seg_model, prior, args, writer):
    with torch.no_grad():
        loss = 0.
        for images, labels in data_loader:
            images = images.to(args.device)
...
def main(args):
    ...
    prior = GatedPixelCNN(args.k, args.hidden_size_prior,
        args.num_layers, n_classes=num_classes).to(args.device)
    optimizer = torch.optim.Adam(prior.parameters(), lr=args.lr)

    best_loss = -1.
    for epoch in range(args.num_epochs):
        train(train_loader, model, seg_model, prior, optimizer, args, writer)
        # The validation loss is not properly computed since
        # the classes in the train and valid splits of Mini-Imagenet
        # do not overlap.
        loss = test(valid_loader, model, seg_model, prior, args, writer)
